...
export function WebHookEvents() {
    ...
    return (
        <div ref={div} className="relative flex items-center justify-between w-full h-10 border rounded-lg hover:cursor-pointer" onClick={(e) => {
                    setEventsOpen(!eventsOpen)
                }}>
            <div className="flex items-center p-2 space-x-2" onClick={(e) => {
                e.stopPropagation()
            }}>
                {elements.map(element => {
                    return (
                        <div key={element.value} className="flex items-center h-6 rounded-lg border border-primary hover:bg-secondary" onClick={() => {
            ...
            <div className="mr-2" onClick={() => {
                ...
                {
                    eventsOpen? <ChevronUp className="w-4 h-4"></ChevronUp> : <ChevronDown className="w-4 h-4"></ChevronDown>
                }
            </div>
            <div className={`${eventsOpen ? "absolute" : "hidden"} w-full max-h-[200px] top-10 bg-accent p-2 rounded-lg overflow-y-scroll`} onClick={(e) => {
                e.stopPropagation()
            }}>
                {languages.map(language => (
                    <div key={language.value} className="flex w-full items-center p-2 hover:bg-primary rounded-lg hover:cursor-pointer" onClick={() => {

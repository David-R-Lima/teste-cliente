  ...
  const createWebhookMutation = useMutation({
    ...
    onError: (error) => {
      if (error instanceof AxiosError) {
        if (error?.code === '404') return error
      }
    },
  })
    ...
    <div className="space-y-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Receba notificações em tempo real por meio de um HTTP POST sempre
            que um evento relacionado às transações desta aplicação acontecer.
          </CardDescription>
        </CardHeader>

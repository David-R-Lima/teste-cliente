            ...
            <AccordionItem value="item-1">
                ...
                <AccordionContent>
                {
                    data.map((property, index) => {
                        const colorClass = property.code >= 200 && property.code < 300 ? 'text-green-500' : 
                           property.code >= 400 && property.code < 500 ? 'text-red-500' : '';

                        return (
                            <div key={index} className="space-y-2 bg-muted p-4 rounded-lg my-4">
                                <div className="flex space-x-2">
                                    <h1 className={`text-bold ${colorClass}`}><strong>{property.code}</strong></h1>
                                </div>
